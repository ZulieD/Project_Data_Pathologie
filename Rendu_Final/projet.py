...
st.write('')
st.write("Nombre moyen d'hospitalisation en France en fonction des différentes niveau de pathologie tous les ans:")
col1, col2, col3 = st.columns(3)
col1.metric("Pathologie de niveau 1", "780 856 924")
col2.metric("Pathologie de niveau 2", "100 062 360")
col3.metric("Pathologie de niveau 3", "670 136 865")
...
with st.expander("Voir explication"):
    st.write("Les valeurs de la colonne 'ntop' représentent la moyenne du nombre d'hospitalisations pour cette pathologie dans un hôpital.")
...
Actuellement, la seule façon d'obtenir un diagnostic rapide est de se rendre à l'hôpital.
...
st.header("Approfondissons notre analyse : ")
...
with st.expander("Voir explication"):
    st.write("Les valeurs de la colonne 'ntop' représentent la moyenne du nombre d'hospitalisations des hommes pour cette pathologie dans un hôpital.")
...
with st.expander("Voir explication"):
    st.write("Les valeurs de la colonne 'ntop' représentent la moyenne du nombre d'hospitalisations des femmes pour cette pathologie dans un hôpital.")
...
# Grouper les données par "patho_niv1" et calculer les moyennes pour chaque groupe de sexe
patho_niv1_means = df.groupby(['patho_niv1', 'sexe'])['ntop'].mean().unstack()

# Parcourir les pathologies et comparer les différences absolues
for patho, row in patho_niv1_means.iterrows():
    mean_h = row[1]  # Moyenne pour les hommes
    mean_f = row[2]  # Moyenne pour les femmes
    
    difference_h = abs(mean_f - mean_h)
    
    if difference_h > 2000:
        if mean_f > mean_h:
            print(f"{patho}: Majorité aux femmes (différence de {difference_h:.2f} hospitalisations)")
        else:
            print(f"{patho}: Majorité aux hommes (différence de {difference_h:.2f} hospitalisations)")
...
st.write(''' Ce code met en évidence une nette distinction entre les types de pathologies prédominantes chez les femmes et chez les hommes, quelle que soit le niveau de gravité de la pathologie.''')
st.write('''Les femmes sont beaucoup plus sujettes aux pathologies liées à l'humeur ou au stress, telles que *les Troubles névrotiques et de l'humeur*, *les Troubles du rythme ou de la conduction cardiaque*,
*les Traitements hypnotiques (hors pathologies)* et *les Traitements antidépresseurs ou régulateurs de l'humeur (hors pathologies)*.''')
st.write('''En revanche, les hommes sont davantage touchés par des pathologies liées au hasard, telles que les cancers ou les troubles cardiaques liés à la vieillesse, comme *l'Insuffisance cardiaque chronique*,
*les Traitements antihypertenseurs*, *les Maladies du foie ou du pancréas*, *les Hospitalisations ponctuelles* et *Pas de pathologies repérées, traitements, maternité, hospitalisations ni traitement antalgique ou anti-inflammatoire*.''')

st.subheader("Mon hypothèse : ")
st.write("On peut donc émettre l'hypothèse que les femmes ont une vie ou une pression importante sur les épaules, ce qui semble avoir un impact plus significatif sur leur santé par rapport aux hommes.")
st.write('''Il n'est pas possible d'établir des certitudes avec cette base de données, mais il serait intéressant d'examiner de manière plus approfondie les modes de vie entre hommes et femmes, ce qui pourrait expliquer ce décalage.''')
st.write('''À mon avis, les femmes subissent une charge mentale bien plus importante que les hommes. Cela a un impact direct sur leur humeur et leur niveau de stress,
car elles s'inquiètent généralement non seulement pour elles-mêmes, mais aussi pour toutes les personnes de leur entourage. Cela se répercute sur leur santé, et les chiffres soutiennent cette hypothèse.''')
